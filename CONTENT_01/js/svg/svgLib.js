var SVGLib = {
    createTag: function(tag, attrs) {
        ...
        $.map(Object.keys(attrs), function(key){ $(obj).attr(key, attrs[key]); });
    ...
    getLinePath: function(o, p1, p2, grid) {
        var d = ["M", 
            ...
            roundDecimal2((o.y + p2.y) * grid)].join(' ');
        return d;
    },
    ...
    drawLine2: function(x1, y1, x2, y2, lineStyle, id, strClass) {
        var attrs = Object.assign(lineStyle, {
            x1: x1,
            y1: y1,
            x2: x2,
    ...
    drawPoint: function(o, p, grid, pointStyle, id, strClass) {
    ...
    drawPoint2: function(cx, cy, pointStyle, id, strClass) {
        var attrs = Object.assign(pointStyle, {
            cx: cx,
            cy: cy,
            id: id,
    ...
    getFacePath: function(o, faceVertices, grid, isOpen) {
        var d = $.map(faceVertices, function(v, id){ 
            return [id == 0? "M": "L", 
                roundDecimal2((o.x + v.x) * grid), 
                roundDecimal2((o.y + v.y) * grid)].join(' ')
        }).join(' ') + (isOpen? "": "z");
        return d;
    },

    getFacePath2: function(faceVertices, isOpen) {
        var d = $.map(faceVertices, function(v, id){ 
            return [id == 0? "M": "L", 
                roundDecimal2(v.x), 
                roundDecimal2(v.y)].join(' ')
        }).join(' ') + (isOpen? "": "z");
        return d;
    },

    getFaceStyle: function(faceVertices) {
        var fill, opacity;
        var minOpacity = 0.2;
    ...
    drawFace: function(o, faceVertices, grid, faceStyle, id, isOpen) {
    ...
    drawFace2: function(faceVertices, faceStyle, id, isOpen) {
        var attrs = Object.assign(faceStyle, {
    ...
    updateStyle: function(el, styles) {
        $.map(Object.keys(styles), function(key){$(el).attr(key, styles[key])});
    },

    getCurveTextPath: function(o, p1, p2, p, grid) {
        var d = [
            "M", roundDecimal2((o.x + p1.x) * grid), roundDecimal2((o.y + p1.y) * grid),
            // "C", r, r, 0, 0, 0, roundDecimal2((o.x + p2.x) * grid), roundDecimal2((o.y + p2.y) * grid)
            "Q", roundDecimal2((o.x + p.x) * grid), roundDecimal2((o.y + p.y) * grid), 
            roundDecimal2((o.x + p2.x) * grid), roundDecimal2((o.y + p2.y) * grid)
        ].join(" ");

        return d;
    },

    drawCurveText: function(o, p1, p2, p, grid, arcStyle, id) {
    ...
    drawRect: function(o, x, y, w, h, grid, rectStyle, id) {
