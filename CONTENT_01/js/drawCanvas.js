...
var g_msgLog = "";
var g_resizeCount = 0;
let g_dragablePosition = {}; // store initial dragable area position
...
const radius = 3;
const x0 = 0;
const y0 = radius * 2 * Math.sqrt(2);
const z0 = 6;
const edgeLength = radius * 2 * Math.sqrt(2);
...
const g_shadowMapSize = [1024, 1024].map((x) => x * 2);

var svgRootWidth = $("#divBody").width();
var svgRootHeight = $("#divBody").height();

const windowRatio = [0.5, 0.68];
let windowWidth = windowRatio[0] * svgRootWidth;
let windowHeight = windowRatio[1] * svgRootHeight;
...
function getGeoShape(vertices, rings) {
    ...
    for (var i = 0; i < rings.length - 1; i++)
        for (var j = 0; j < n; j++) {
            ...
            geoShape.faces.push(
                new THREE.Face3(idMap[j4], idMap[j3], idMap[j2])
            );
            geoShape.faces.push(
                new THREE.Face3(idMap[j4], idMap[j2], idMap[j1])
            );
        }
...
function drawCanvasShape(vertices, faces, rings, id) {
    ...
    if (g_currentShapes) {
        g_currentShapes.forEach((sh) => singleShape.remove(sh));
    ...
    const edgesMesh = new THREE.LineSegments(
        new THREE.EdgesGeometry(geoShape, 0.5),
    ...
    const edgesMeshDashed = new THREE.LineSegments(
        new THREE.EdgesGeometry(geoShape),
        new THREE.LineDashedMaterial({
            color: color,//"#005aff", // "#005aff",
            transparent: true,
            opacity: 0.2,
            scale: 30,
            linewidth: 1,
    ...
    g_currentShapes.forEach((sh) => singleShape.add(sh));
...
function showCanvas() {
    ...
    $("#container")
        .parent()
        .css("left", $("#container").parent().attr("origin-left"));
}
