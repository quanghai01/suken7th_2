...
function Shape3D(type, origin, grid, id, translateVector, data) {
  ...
  for (var i = 0; i <= n0; i++) {
    halfCircularVertices.push(new Point3D(h / 2 * Math.cos(Math.PI / 2 + Math.PI * i / n0), h / 2 * Math.sin(-Math.PI / 2 + Math.PI * i / n0), 0));
  }

  for (var i = 0; i <= n1; i++) {
    var O = new Point3D(-h / 2, 0, 0);
    circleVertices.push(O.add(new Point3D(h / 4 * Math.cos(Math.PI + 2 * Math.PI * i / n0), h / 4 * Math.sin(-Math.PI + 2 * Math.PI * i / n0), 0)));
  }

  for (var i = 0; i <= n3; i++) {
    vertices11.push(new Point3D(w * Math.cos(Math.PI / 2 + Math.PI * i / (2 * n3)), w * Math.sin(-Math.PI / 2 + Math.PI * i / (2 * n3)), 0));
  }
  ...
  var verticesData = [[new Point3D(0, -h / 2, 0), new Point3D(-w, -h / 2, 0), new Point3D(-w, h / 2, 0), new Point3D(0, h / 2, 0)], [new Point3D(0, -h / 2, 0), new Point3D(-w, h / 2, 0), new Point3D(0, h / 2, 0)], halfCircularVertices, [new Point3D(0, -h / 2, 0), new Point3D(-w, h / 4, 0), new Point3D(0, h / 2, 0)], [new Point3D(0, -h / 2, 0), new Point3D(-w / 2, -h / 2, 0), new Point3D(-w, h / 2, 0), new Point3D(0, h / 2, 0)], [new Point3D(0, -h / 4, 0), new Point3D(-w / 2, -h / 2, 0), new Point3D(-w, -h / 2, 0), new Point3D(-w, h / 2, 0), new Point3D(0, h / 2, 0)], [new Point3D(-w / 4, -h / 2, 0), new Point3D(-w, -h / 2, 0), new Point3D(-w, h / 2, 0), new Point3D(-w / 4, h / 2, 0), new Point3D(-w / 4, -h / 2, 0)], [new Point3D(0, -h / 2, 0), new Point3D(-w / 2, -h / 2, 0), new Point3D(-0.001, -h / 4, 0), new Point3D(-w, h / 2, 0), new Point3D(0, h / 2, 0)], [new Point3D(0, -h / 2, 0), new Point3D(-w, -h / 4, 0), new Point3D(-w, h / 2, 0), new Point3D(0, h / 2, 0)], [new Point3D(0, -h / 2, 0), new Point3D(-w, -h / 2, 0), new Point3D(0, h / 2, 0)], vertices11, // vertices12,
  vertices12_2D, // [
  ...
  if (type == CONST.SHAPE_TYPES.SHAPE_2D) {
    ...
    var vertices = [];
    var initVertices = this.id <= 13 ? verticesData[this.id - 1] : data.initVertices.map(function (v) {
      return v.multiple(h0);
    });
...
console.log(vertices)
  ...
  } else if (type == CONST.SHAPE_TYPES.SHAPE_3D && (this.id == 7 || this.id == 13)) {
    ...
    for (var i = 0; i < l; i++) {
      ...
      for (var j = 0; j < n; j++) {
        ...
        this.vertices.push(new Point3D(p.x * sinArr[j], p.y, p.x * cosArr[j]));
      }
    ...
    if (this.id > 13) initVertices = data.initVertices.map(function (v) {
      return v.multiple(h0);
    });
    ...
    for (var i = 0; i < l; i++) {
      ...
      for (var j = 0; j < n; j++) {
        ...
        if (i < l - 1) {
          this.faces.push([pCount + j + n, pCount + (j + 1) % n + n, pCount + (j + 1) % n, pCount + j]);
        }
      }
  ...
  if (type == CONST.SHAPE_TYPES.SHAPE_3D) {
    ...
    if (singleShape) {
      singleShape.rotation.set((g_shape2D.rotateTotalY || 0 - 0) * Math.PI / 180, -(g_shape2D.rotateTotalX || 0 * Math.PI) / 180, singleShape.rotation.z || 0);
    }
  }
  ...
  if (this.id == 12) {
    ...
    this.Y1 = MathLib.getPointByRatio(Q, P, -0.18); // this.rotateY(-2);
...
Shape3D.prototype.rotateY2 = function (alpha) {
  ...
  this.alphaY += alpha;
  if (this.alphaY < 0) this.alphaY += 360;else if (this.alphaY >= 360) this.alphaY -= 360; // var A = this.vertices[0];
...
Shape3D.prototype.getIntersectionLine = function (P, Q, lineArrayList) {
  ...
  while (i < ratios.length - 1) {
    ...
    while (ratios[j] - ratios[i] < 0.0001) {
      j++;
    }
  ...
  if (1 - ratios2[ratios2.length - 1] < 0.0001) ratios2[ratios2.length - 1] = 1;else ratios2.push(1);
  return ratios2;
};
...
Shape3D.prototype.updateLayer = function (n) {
  ...
  return; // if (this.layerCount > 10)
  //     return;

  var pre = 0;

  for (var j = this.layerCount - 1; j >= Math.max(this.layerCount - n, 0); j--) {
    pre++;
    console.log("render2d", this.layerCount, j);
    var id2 = [this.shapeId, "face_layer", j].join("_");
    $("#".concat(id2)).attr("opacity", 1 - 0.7 * pre / n);
  }

  if (this.layerCount >= n + 1) {
    var id3 = [this.shapeId, "face_layer", this.layerCount - n - 1].join("_");
    $("#".concat(id3)).remove();
  }
...
Shape3D.prototype.updateLayer2 = function (n) {
  ...
  for (var j = this.layerCount - 2; j >= Math.max(this.layerCount - n, 0); j--) {
    var id2 = [this.shapeId, "face_layer", j].join("_");
    $("#".concat(id2)).remove();
...
Shape3D.prototype.checkEdge = function (p1, p2, faces1) {
  var faces2 = arguments.length > 3 && arguments[3] !== undefined ? arguments[3] : [];
  var M = MathLib.getPointByRatio(p1, p2, 0.5);
  console.log("checkEdge", M, faces1, faces2);
  if (MathLib.pointInsidePolygon(M, faces1) || MathLib.pointInsidePolygon(M, faces2)) return false;
  return true;
};
...
Shape3D.prototype.render = function () {
  ...
  var _vertices = this.vertices.map(function (v) {
    return v.add(_this.translateVector);
  });
  ...
  if (this.type == CONST.SHAPE_TYPES.SHAPE_2D) {
    for (var i = 0; i < 72; i++) {
      var vertices = _vertices.slice(i * this.sliceCount, (i + 1) * this.sliceCount); // face layer 0
      ...
      if (el.length > 0) {
        var d = SVGLib.getFacePath(this.origin, vertices, this.grid, isOpen);
        $(el).attr("d", d);
        ...
        var svgFace = SVGLib.drawFace(this.origin, vertices, this.grid, faceStyle, id, isOpen);
        this.layerDom.append(svgFace);
        var id2 = "#".concat([this.shapeId, "face_layer", i].join("_"));
    ...
    if (el.length > 0) {
      var d = SVGLib.getLinePath(this.origin, this.Y1.add(this.translateVector), this.Y2.add(this.translateVector), this.grid);
      $(el).attr("d", d);
    } else {
      var faceStyle = {
        stroke: "#000",
      ...
      var svgLine = SVGLib.drawLine(this.origin, this.Y1.add(this.translateVector), this.Y2.add(this.translateVector), this.grid, faceStyle, id);
      this.shapeDom.append(svgLine);
    } // sort layers


    var allLayers = _($(".shape_layer")).map(function (el) {
      return {
        el: el,
    ...
    var layerDomId = "#".concat(["layer_dom", this.shapeId].join("_"));
...
Shape3D.prototype.drawLabel = function (label, position) {
  ...
  if (el.length > 0) {
    ...
    var svgText = SVGLib.drawText(this.origin, vertex.x * 1.05, vertex.y * 1.05, label, this.grid, style, ["text_vertex", label, position].join("_"));
    var flagDom = $("#flag");
...
Shape3D.prototype.drawPoint = function (p, id, fillColor, radius) {
  ...
  if (el.length > 0) {
    ...
    var strClass = "shape-element-".concat(this.id);
    var svgPoint = SVGLib.drawPoint(this.origin, p, this.grid, pointStyle, id, strClass);
